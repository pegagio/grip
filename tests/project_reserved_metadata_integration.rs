...
fn project_root_tree_prunes_grip_metadata_before_ignore_evaluation() {
    ...
    let added = fixture.command(&["mapping", "add", "tree", ".", "~"]);
    assert!(
    ...
    let inspected = fixture.command(&["--output=json", "mapping", "inspect", "."]);
    ...
    assert!(records.iter().any(|record| {
        record["relative_path"]["display"] == "payload" && record["category"] == "eligible"
    }));
    assert!(!records.iter().any(|record| {
...
fn metadata_sources_and_selectors_are_rejected_without_descriptor_changes() {
    ...
    for source in [".grip", ".grip/config.toml", "payload/../.grip"] {
        let added = fixture.command(&["mapping", "add", "file", source, "~/target"]);
        assert!(!added.status.success(), "source {source} was accepted");
    }
    let inspected = fixture.command(&["mapping", "inspect", ".grip"]);
    assert!(!inspected.status.success());
