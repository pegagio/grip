...
impl ProjectContext {
    ...
    /// Select a project whose live descriptor must be restored from an exact
    /// project-local registry recovery reference.
    pub(crate) fn select_for_registry_recovery(
        selection: ProjectSelection,
        user_home: UserHome,
        reference: &crate::recovery::model::RecoveryRef,
    ) -> Result<Self, GripError> {
        if !matches!(
            reference,
            crate::recovery::model::RecoveryRef::Registry { .. }
        ) {
            return Err(project_error(
                "invalid_project_metadata",
                "only registry recovery can bootstrap a missing project descriptor",
            ));
        }
        let root = recovery_root(selection)?;
        load_recovery_context(&root, user_home, reference)
    }
}

fn recovery_root(selection: ProjectSelection) -> Result<PathBuf, GripError> {
    match selection {
        ProjectSelection::Explicit(path) => fs::canonicalize(path)
            .map_err(|error| GripError::from_io("could not resolve project root", error)),
        ProjectSelection::Discovered(cwd) => {
            let canonical = fs::canonicalize(cwd).map_err(|error| {
                GripError::from_io("could not resolve invocation directory", error)
            })?;
            let mut roots = Vec::new();
            for ancestor in canonical.ancestors() {
                match fs::symlink_metadata(ancestor.join(METADATA_DIRECTORY)) {
                    Ok(_) => roots.push(ancestor.to_path_buf()),
                    Err(error) if error.kind() == std::io::ErrorKind::NotFound => {}
                    Err(error) => {
                        return Err(GripError::from_io(
                            "could not inspect project ancestor for recovery",
                            error,
                        ));
                    }
                }
        ...
        ProjectSelection::Independent => Err(project_error(
            "invalid_project_root",
            "registry recovery requires a project",
        )),
    }
}
...
fn load_recovery_context(
    ...
    user_home: UserHome,
    reference: &crate::recovery::model::RecoveryRef,
) -> Result<ProjectContext, GripError> {
    ...
    let descriptor_bytes = crate::mutation::filesystem::read_private_file(&located.payload_path)?;
    let crate::recovery::model::RecoveryRef::Registry { digest } = reference else {
        unreachable!()
    };
    if format!("{:x}", Sha256::digest(&descriptor_bytes)) != *digest {
    ...
    let descriptor = decode_descriptor(descriptor_text)?;
    ...
    let recovered_metadata = safe_metadata(&located.payload_path, false, "recovered descriptor")?;
    let evidence = ProjectEvidence {
        ...
        descriptor: node_evidence(located.payload_path, &recovered_metadata),
