...
Tree membership is defined by source-side discovery and `.gripignore`. The project-root `.gripignore` is a global policy for every tree mapping. A source-root or nested `.gripignore` takes precedence within its narrower source scope, following Gitignore-compatible matching. Policy files are never mapping payload. Ignored entries are outside Grip's active membership. A direct ignore-file edit is therefore a policy change: it prunes relevant state and a later unignore is treated as new membership rather than a restored history. The sole exception is explicit `pull -a|--adopt DESTINATION`: it admits one destination-only regular file strictly below a declared tree mapping, plus only missing source ancestors required to place it. It neither enumerates siblings nor adds a mapping. An ignored target requires `--force`; that override changes no policy and emits a verified `.gripignore` exemption recommendation for future ordinary discovery. It never bypasses source existence, ownership, topology, metadata, or stale-evidence safety checks.
...
When an otherwise blocked `push`, `pull`, or `sync` contains only initial collisions or ordinary divergent conflicts, Grip follows each displayed path pair with the same two concrete force choices. A technical or mixed block instead tells the operator to run `grip status` for current detailed safety information. A blocked no-selector `push --force` identifies the aggregate operation and lists its available blocker reasons with managed path evidence before the same status guidance. Default human mutation output omits raw planner identifiers, winner fields, action milestones, recovery state, baseline authority, and operation-record evidence; the category, exit behavior, and JSON result remain unchanged. If execution fails after it starts, Grip preserves the completed-action count and directs the operator to `grip status` before retrying.

Source selectors for mapping commands, `status`, `diff`, and `sync` retain their project-relative interpretation. `pull` selectors are destination-space by default, while `pull -s` and `pull --source` select in source space. A relative source selector for `push`, including dry-run and forced forms, is resolved from the process current working directory and must remain inside the selected project after traversal and symlink checks. Absolute source selectors remain invalid. Destination selectors retain destination-space interpretation where supported.

`push -f PATH` and `pull -f PATH` deliberately select a winner for exactly one managed entry. `push -f` makes the source authoritative; `pull -f` makes the destination authoritative. A present selected winner restores a missing peer; for an existing simple one-sided deletion, a selected absent winner propagates intentional absence by removing the unchanged losing endpoint. For an unresolved exact destination leaf link, only exact source-winning `push -f PATH` may replace the revalidated link object through atomic sibling publication; it never acts on the former target. Source links, destination-link ancestors, pull-side replacement, aggregate selection, and ordinary operations remain blocked. Force is never mapping-wide, tree-wide, ambiguous, or available through `sync`.

## State and safety

Grip stores portable mapping declarations in the project and mutable current evidence in `.grip/state/`. State contains active-membership baselines only; it is pruned when a mapping is removed or an entry leaves membership. For an unequal add, a private mapping-scoped publication fence is created before the descriptor and State V4 candidate are published and is cleared only after both verify. State rebinding distinguishes mapping-relevant project location and resolved topology from output-only descriptor settings, so changing a diff-tool profile alone does not invalidate accepted payload evidence. Grip uses temporary staging, lock-held revalidation, post-publication verification, and atomic baseline publication to make mutations safe.
