...
This continuation records Features 029 through 037 from the current roadmap ledger. (S003)
...
37. **037 — Initial-Match Baseline Synchronization (`verified`):** makes a selected `sync` path accept one complete equivalent unbaselined managed entry without copying payloads. It excludes no-selector, tree-root, and multi-entry scopes, does not add a baseline command, and preserves conflict, force, ownership, and safety boundaries. Its implementation debrief recorded `PROCEED` with no findings. (S003)
