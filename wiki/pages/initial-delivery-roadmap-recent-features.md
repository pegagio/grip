...
26. **026 — Exact Destination Symlink Replacement (`planned`):** will let nonmutating `grip add` record mappings whose exact managed destination leaves are symlinks, report each unresolved link precisely, and let exact-entry source-winning `push --force` replace only the verified link object without following its target. Source symlinks, symlink ancestors, aggregate force, pull-side replacement, and mutation during `add` remain outside scope. (S003)

Features 021 through 025 establish measured large-file responsiveness, exact mapping replacement, missing-peer restoration, local release preparation, and safe contained-source tree mappings. Planned Feature 026 extends that chain with explicit exact destination-leaf symlink replacement while retaining no-follow filesystem safety. (S003)
