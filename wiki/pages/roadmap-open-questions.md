...
- **003 (resolved for discovery):** `.gripignore` follows the enumerated Gitignore-compatible contract and remains policy-only; empty directories enter discovered membership. Link policy flows forward through Features 009 and 026. (S003)
...
- **009 (resolved):** supported equality includes independent file and directory metadata; current macOS on APFS is the qualification boundary; exact path bytes are preserved; and observed case and Unicode behavior blocks incompatible identities rather than normalizing them. (S003)
...
- **026 (partially resolved for links):** source links and symlink ancestors remain unsupported, while an exact managed destination-leaf symlink may be observed without following it, admitted by nonmutating `add`, and replaced only by exact-entry source-winning forced push. Feature 026 is verified; broader link-object synchronization remains undecided. (S003)
