...
No-selector `grip push --force` is an explicit aggregate source-winning operation across all managed project entries, while a supplied selector retains the exact-entry force contract. `grip push --force --dry-run` previews that aggregate without payload or accepted-state mutation. Planned Feature 028 makes `grip diff` invoke a configurable external program, with `diff` as the default and no shell-string execution. [Initial delivery roadmap future features](./initial-delivery-roadmap-future-features.md) records their bounded scope. (S003) (S004)
